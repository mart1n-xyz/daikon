...
function getContractDataFromDeployments() {
  ...
  for (const chainName of getDirectories(DEPLOYMENTS_DIR)) {
    ...
    for (const contractName of getContractNames(`${DEPLOYMENTS_DIR}/${chainName}`)) {
      const filePath = `${DEPLOYMENTS_DIR}/${chainName}/${contractName}.json`;
      console.log(`Reading ABI file: ${filePath}`);
      const { abi, address, metadata } = JSON.parse(fs.readFileSync(filePath).toString());
